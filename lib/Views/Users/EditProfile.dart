...
Future<void> _updateUserProfile() async {
  ...
  // Créer une instance de Const pour l'URL de l'API
  final constInstance = Const();
  ...
  try {
    ...
    print("Sending update data: $updateData"); // Log pour débogage
    ...
    if (response.statusCode == 200) {
      ...
      print("Received updated user data: $updatedUserData"); // Log pour débogage
      ...
      print("Updated user: ${updatedUser.name}, ${updatedUser.email}"); // Log
      ...
      print("UI state updated: ${currentUser!.name}, ${currentUser!.email}"); // Log
      ...
      print("Error response: ${response.body}"); // Log pour débogage
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
    ...
    print("Error updating profile: $error"); // Log pour débogage
    Navigator.of(context).pop();
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        content: Text("Erreur de connexion : $error"),
  ...
  onPressed: isSaveButtonEnabled
      ? () async {
          ...
          try {
            // Appeler auth.updateUser
            final updatedData = await auth.updateUser(
              userId!,
              _nameController.text,
              _emailController.text,
              _phoneController.text,
            ...
            print("Updated data from API: $updatedData"); // Log pour débogage

            if (updatedData != null) {
              // Créer un nouvel objet User avec les données mises à jour
              final updatedUser = User(
                id: userId!,
                name: updatedData['name'] ?? _nameController.text,
                email: updatedData['email'] ?? _emailController.text,
                phoneNumber:
                    updatedData['phoneNumber'] ?? _phoneController.text,
              );

              print("New user object: ${updatedUser.name}, ${updatedUser.email}"); // Log
              ...
              print("UI updated: ${currentUser!.name}, ${currentUser!.email}"); // Log
              ...
              throw Exception('No data returned from updateUser');
            }
          } catch (e) {
            print("Error updating user: $e"); // Log pour débogage
            setState(() {
              isLoading = false;
            });
            ScaffoldMessenger.of(context).showSnackBar(
  ...
  style: ElevatedButton.styleFrom(
    backgroundColor: isSaveButtonEnabled ? const Color(0xFF29E33C) : Colors.grey,
